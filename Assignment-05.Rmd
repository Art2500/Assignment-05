...
wget ("https://dataverse.harvard.edu/api/access/datafile/:persistentId?persistentId=doi:10.7910/DVN/L20LOT/FZLQRQ", destfile ="./raw_data/Countries-Deaths.tsv")
...
```{r}
read_csv("./output/GFI_total_deaths_by_date.csv") %>% group_by(Country,Date) %>% mutate(Daily_Deaths=Total_Deaths - lag(Total_Deaths,n=1,default = NA)) %>% select(Total_Deaths) %>% write_csv("./output/GFI_daily_deaths_by_date.csv")
```
...
```{r}
DailyDeathTime<-read_csv("./output/GFI_daily_deaths_by_date.csv")
ggplot(DailyDeathTime, aes(x=Date, y=Total_Deaths, color=Country)) + geom_line(size=1.5) + scale_y_log10() +theme_linedraw() + labs(y="Daily COVID-19 Deaths")
```
